...
public class enemyia3 : MonoBehaviour
{
    ...
    void Update()
    {
        if (Math.Abs(Target.gameObject.transform.position.x - this.transform.position.x) < 90 && Math.Abs(Target.gameObject.transform.position.z - this.transform.position.z) < 90)
        {
            AttackPlayer();
